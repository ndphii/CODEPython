...
def inputPlayerLetter():
    ...
    while not (letter == 'X' or letter == 'O'):
        print('Bạn muốn chơi quân X hay O?')
        letter = input().upper()
...
def getPlayerMove(board):
    ...
    while move not in '1 2 3 4 5 6 7 8 9'.split() or not isSpaceFree(board, int(move)):
        print('chọn vị trí đánh trên bàn cờ ? (1-9)')
        move = input()
    return int(move)
...
while True:
    ...
    print('bạn muốn chơi lại không? (yes or no)')
